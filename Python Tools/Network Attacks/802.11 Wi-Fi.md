# Python 802.11 Wi-Fi Deauth Attack Tool with Scapy
...
## Overview
This is a comprehensive 802.11 Wi-Fi deauthentication attack tool built with Scapy. It demonstrates how attackers can disconnect clients from Wi-Fi networks by sending forged deauth frames. The tool includes network scanning, targeted attacks, and various advanced features for authorized security testing and educational purposes.
...
1. **Deauthentication Attacks**:
   ...
   - Continuous or limited packet count
   - Configurable deauth reason codes

2. **Network Reconnaissance**:
   - AP scanning and discovery
   - Client enumeration
   - Channel detection
   - Signal strength monitoring

3. **Advanced Attack Techniques**:
   - Channel hopping to follow APs
   - Multiple AP simultaneous attack
   - Scan-then-attack workflow
   - Rate control (packets per second)

4. **802.11 Frame Support**:
   - Beacon frame parsing
   - Deauth frame crafting
   - RadioTap headers for injection
   - Frame type/subtype handling

## Script Structure

### Class: `WiFiDeauthAttack`
Main class implementing deauth attacks:

- **`scan_networks()`**: Discovers APs and clients
- **`create_deauth_packet()`**: Crafts deauth frames
- **`attack_targeted()`**: Single AP/client attack
- **`attack_broadcast()`**: Multiple AP attack
- **`channel_hopper()`**: Channel switching for wide coverage
- **`get_encryption()`**: Determines AP encryption type
- **`display_scan_results()`**: Shows discovered networks

### Class: `WiFiScanner`
Dedicated scanning functionality:

- **`packet_handler()`**: Processes captured frames
- **`scan()`**: Main scanning loop
- **`display_results()`**: Shows scan findings

## 802.11 Frame Structure

### Deauthentication Frame
```
RadioTap Header (variable)
  - Timestamp
  - Flags
  - Rate
  - Channel
...
# On Debian/Ubuntu
sudo apt install python3-scapy python3-colorama python3-tqdm
...
sudo python3 wifi_deauth.py [options]
```
...
sudo python3 wifi_deauth.py --interface wlan0 --enable-monitor --scan
```
...
```bash
# Scan for networks
sudo python3 wifi_deauth.py --interface wlan0mon --scan

# Scan with verbose output
sudo python3 wifi_deauth.py --interface wlan0mon --scan --verbose
```
...
# Limited number of packets (1000)
sudo python3 wifi_deauth.py --interface wlan0mon --bssid 00:11:22:33:44:55 --count 1000

# Targeted client deauth
sudo python3 wifi_deauth.py --interface wlan0mon --bssid 00:11:22:33:44:55 --client AA:BB:CC:DD:EE:FF
```

### Advanced Attacks
```bash
# Scan then attack all networks
sudo python3 wifi_deauth.py --interface wlan0mon --scan --attack-all --count 5000

# Channel hopping attack (follows AP on channel changes)
sudo python3 wifi_deauth.py --interface wlan0mon --bssid 00:11:22:33:44:55 --channel-hop

# High-speed attack (50 packets per second)
sudo python3 wifi_deauth.py --interface wlan0mon --bssid 00:11:22:33:44:55 --rate 50

# Different deauth reason (8 - AP leaving)
sudo python3 wifi_deauth.py --interface wlan0mon --bssid 00:11:22:33:44:55 --reason 8
```
...
| 2 | Previous authentication no longer valid | Force re-auth |
| 3 | Station leaving (default) | AP shutdown simulation |
| 4 | Inactivity | Client timeout simulation |
| 5 | AP unable to handle stations | Overload simulation |
| 6 | Class 2 frame from non-authenticated | Security test |
| 7 | Class 3 frame from non-associated | Security test |
| 8 | Station leaving | Client disconnection |
| 9 | Station not authenticated | Auth bypass test |

## How Deauth Attacks Work

### Normal 802.11 Authentication Flow
```
1. Client sends Authentication Request
2. AP sends Authentication Response
...
### Deauth Attack Flow
```
1. Attacker sends Deauth frame spoofed as AP
2. Frame contains reason code (usually 3 or 8)
3. Client receives frame, believes it's from AP
4. Client disconnects from network
5. Client may attempt to reconnect
6. Attacker can capture handshake during reconnection
```

### Frame Spoofing
The attack works because:
- Deauth frames are not encrypted
- Clients trust deauth frames from APs
- No authentication on management frames
- Attacker can forge source MAC address
...
2. **Network Disruption**: Deauth attacks:
   ...
   - Are detectable by WIDS/WIPS

3. **Legal Consequences**: Unauthorized Wi-Fi attacks are illegal in most countries under:
...
4. **Rogue AP Detection**:
   ```bash
   # Look for APs with same BSSID as legitimate AP
   airodump-ng wlan0mon --bssid 00:11:22:33:44:55
   ```
...
| No packets sent | Check interface, antenna, proximity |
| No networks found | Adjust scan time, check channel |
| Attack ineffective | Increase rate, check target proximity |

## Performance Optimization

- **Packet Rate**: 10-20 pps usually sufficient
- **Distance**: Stay within 50-100 meters of target
- **Channel**: Ensure you're on correct channel
- **Antenna**: Use external antenna for better range

## Limitations

- Requires proximity to target (WiFi range)
- Does not work on 802.11w protected networks
- Some clients ignore deauth frames
- APs may have rate limiting
- Can be detected by WIDS/WIPS
...
To test defenses:
1. Set up AP with 802.11w enabled
2. Configure WIDS (e.g., Kismet)
3. Attempt deauth attacks
4. Observe protection effectiveness
