# Python DHCP Starvation Attack Tool
...
## Overview
This is a comprehensive DHCP starvation attack tool built with Scapy for network packet manipulation. It demonstrates how an attacker can exhaust a DHCP server's IP address pool by flooding it with DHCPDISCOVER packets using unique fake MAC addresses.

## Key Features

1. **DHCP Starvation Techniques**:
   - High-speed DHCPDISCOVER flooding
   - Unique MAC address generation per request
   - Configurable packet rate
   - Maximum lease targeting

2. **MAC Address Generation**:
   - Completely random MAC addresses
   - Prefix-based generation (vendor-specific)
   - Duplicate avoidance
   - Realistic OUI prefixes

3. **Response Monitoring**:
   - Sniffs DHCP OFFER responses
   - Tracks ACK/NAK messages
...
### Class: `DHCPStarvationAttack`
The main class handling all DHCP starvation operations:

- **`__init__()`**: Initializes network interface and attack parameters
- **`generate_mac()`**: Creates unique MAC addresses
- **`create_dhcp_discover()`**: Builds DHCPDISCOVER packets
- **`create_dhcp_request()`**: Creates DHCPREQUEST for verification
- **`dhcp_offer_handler()`**: Processes DHCP OFFER responses
- **`attack_worker()`**: Main flooding thread
- **`start_sniffer()`**: Captures DHCP server responses
- **`check_dhcp_range()`**: Queries DHCP server configuration
- **`show_statistics()`**: Displays attack metrics
...
sudo python3 dhcp_starvation.py [options]
```
...
### Basic Attacks
```bash
# Exhaust 1000 leases at 10 packets/sec
sudo python3 dhcp_starvation.py -i eth0 --max-leases 1000

# High-speed attack (50 packets/sec)
sudo python3 dhcp_starvation.py -i eth0 --rate 50 --max-leases 5000

# Time-limited attack (60 seconds)
sudo python3 dhcp_starvation.py -i eth0 --rate 20 --timeout 60
```

### Targeted Attacks
```bash
# Target specific DHCP server
sudo python3 dhcp_starvation.py -i eth0 --server 192.168.1.1 --max-leases 1000

# Check server first, then attack
sudo python3 dhcp_starvation.py -i eth0 --check --max-leases 1000
```

### Advanced Options
```bash
# With specific MAC prefix
sudo python3 dhcp_starvation.py -i eth0 --mac-prefix "00:11:22" --max-leases 500

# Verbose monitoring
sudo python3 dhcp_starvation.py -i eth0 --max-leases 500 -v

# Custom statistics interval
sudo python3 dhcp_starvation.py -i eth0 --max-leases 1000 --stats-interval 50
```

## DHCP Protocol Understanding
...
4. Server sends DHCPACK/DHCPNAK
5. Client configures interface
```

### Attack Mechanism
The tool exploits the first step:
- Sends many DHCPDISCOVER packets
- Each with unique MAC address
- Server reserves IP for each
- Eventually pool exhausts
- Legitimate clients get no IP

## MAC Address Generation

### Methods:
1. **Random**: Fully random with unicast/local bits set
2. **Prefix-based**: Fixed OUI + random suffix
3. **OUI database**: Real vendor prefixes

### Format:
```
Example: 02:00:00:12:34:56
- First byte: 02 (locally administered, unicast)
- Rest: Random or prefix-based
```
...
2. **Network Disruption**: DHCP starvation can:
   - Prevent new devices from joining network
   - Cause service disruptions
   - Trigger security alerts

3. **Detection**: Modern networks have protections:
   - DHCP snooping
   - Port security
   - Rate limiting
...
1. **Enable DHCP Snooping**:
   ...
   ip dhcp snooping trust interface Gi1/0/1
   ```
...
4. **Monitor DHCP Traffic**:
   - Track DHCPDISCOVER rates
   - Alert on anomalies
   - Log MAC address usage

### Signs of Attack:
- High rate of DHCPDISCOVER packets
- Rapid MAC address changes
- DHCP pool exhaustion alerts
- Many clients failing to get IPs
...
| No packets sent | Check interface and network |
| No OFFERs received | Server may have protection |
| Low success rate | Increase rate or check server |
| Duplicate MACs | Tool avoids duplicates automatically |
| Sniffer not working | Check BPF filter and interface |

## Performance Considerations

- **Rate vs. Detection**: Higher rates increase success but also detection risk
- **Network Impact**: Can saturate network with broadcast traffic
- **CPU Usage**: Packet generation and sniffing are intensive
- **Memory**: MAC tracking set grows with max-leases

## Limitations

- Requires being on same network segment
- Won't work with DHCP snooping enabled
- Cannot exhaust pools with very large ranges (>10,000 IPs)
- May trigger DoS protections
- Some switches filter DHCP broadcasts

## Educational Use

This tool is excellent for learning about:
- DHCP protocol internals
- Network denial of service concepts
...
To test defenses:
1. Set up DHCP server with small pool (10 IPs)
2. Enable DHCP snooping on switch
3. Configure port security
4. Run attack and observe failures
5. Tune protections based on results
