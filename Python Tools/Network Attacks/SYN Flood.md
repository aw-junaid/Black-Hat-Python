# Python SYN Flood / DoS Attack Scripts
...
## Overview
This is a comprehensive SYN flood and DoS attack suite built with Scapy and raw sockets. It demonstrates various denial-of-service techniques for authorized security testing and educational purposes. The tool includes multiple attack vectors, performance optimizations, and monitoring capabilities.
...
2. **Performance Options**:
   ...
   - Raw socket high-performance mode
   - Multi-threaded workers
   - Configurable packet rates

3. **Advanced Features**:
   - IP spoofing with custom ranges
   - Dynamic source port selection
   - TCP option configuration
   - Response monitoring (SYN-ACK/RST)
   - Bandwidth tracking

4. **Attack Variations**:
   - Time-limited attacks
   - Packet-count limited attacks
...
### Class: `SYNFloodAttack`
Main class for SYN flood implementation:

- **`__init__()`**: Initializes attack parameters
- **`generate_source_ips()`**: Creates spoofed IP pool
- **`generate_syn_packet_scapy()`**: Builds SYN packets with Scapy
- **`create_ip_header()`**: Raw IP header construction
- **`create_tcp_header()`**: Raw TCP header with options
- **`attack_worker_scapy()`**: Scapy-based attack thread
- **`attack_worker_raw()`**: Raw socket attack thread
- **`response_listener()`**: Monitors target responses
- **`monitor_bandwidth()`**: Tracks throughput

### Class: `DOSSimulator`
Comprehensive DoS testing suite:

- **`icmp_flood()`**: Ping flood attack
...
pip install netifaces colorama psutil

# On Debian/Ubuntu
sudo apt install python3-scapy python3-netifaces python3-colorama python3-psutil
```

### Root Privileges
```bash
# Script requires root for packet injection
sudo python3 syn_flood.py [options]
```
...
```bash
# Basic SYN flood (1000 pps, 60 seconds)
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --rate 1000 --duration 60

# High-performance raw socket attack
sudo python3 syn_flood.py --target 192.168.1.100 --port 443 --rate 10000 --method raw

# Spoofed IP attack with custom range
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --spoof-ip --src-ip-range 10.0.0.0/24

# Limited packet count
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --packets 10000

# With response monitoring
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --rate 5000 -v
```
...
```bash
# Comprehensive tests
sudo python3 syn_flood.py --target 192.168.1.100 --test-suite --duration 30

# UDP flood only
sudo python3 syn_flood.py --target 192.168.1.100 --udp-test --rate 2000 --duration 60

# ICMP flood only
sudo python3 syn_flood.py --target 192.168.1.100 --icmp-test --rate 5000 --duration 30

# Slowloris attack
sudo python3 syn_flood.py --target 192.168.1.100 --slowloris --duration 120
```
...
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --src-port-min 10000 --src-port-max 20000

# Custom TCP parameters
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --window 4096 --ttl 128

# Mixed method (scapy + raw)
sudo python3 syn_flood.py --target 192.168.1.100 --port 80 --method mixed --rate 20000
```
...
- **Window Scale**: Scaling factor

## Performance Optimization

### Rate Calculation
- **Single thread**: Up to 1,000 pps
- **Multi-thread (4)**: Up to 10,000 pps
- **Raw sockets**: 50,000+ pps
- **Mixed mode**: 100,000+ pps
...
2. **Network Disruption**: SYN floods can:
   - Exhaust connection queues
   - Cause service unavailability
   - Trigger DDoS protections
   - Alert security systems

3. **Legal Consequences**: Unauthorized DoS attacks are illegal in most jurisdictions and can result in:
   - Criminal charges
   - Civil lawsuits
   - Imprisonment
...
| Low packet rate | Increase workers, use raw sockets |
| No SYN-ACKs | Target may have protection |
| Network congestion | Reduce rate, use smaller packets |
| Raw socket errors | Check capabilities, interface |
| Memory issues | Reduce source IP pool size |

## Performance Benchmarks

| Method | Rate (pps) | Bandwidth | CPU Usage |
|--------|------------|-----------|-----------|
| Scapy (1 thread) | 1,000 | 0.5 Mbps | 10% |
| Scapy (4 threads) | 4,000 | 2 Mbps | 30% |
| Raw (1 thread) | 10,000 | 5 Mbps | 15% |
| Raw (4 threads) | 40,000 | 20 Mbps | 50% |
| Mixed | 100,000+ | 50+ Mbps | 80% |

## Limitations

- Requires root privileges
- Raw sockets may not work on all platforms
- Limited by network interface speed
- Spoofed IPs may not route back
- Modern OS have SYN flood protection
- Cloud targets have DDoS mitigation

## Educational Use

This tool is excellent for learning about:
- TCP/IP protocol internals
- Network DoS attack techniques
...
To test defenses:
1. Set up test server with web service
2. Configure SYN cookies
3. Implement rate limiting
4. Run SYN flood at increasing rates
5. Observe protection mechanisms
6. Tune defenses based on results
