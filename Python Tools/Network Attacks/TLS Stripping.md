...
## Overview
This is a comprehensive SSL/TLS stripping tool suite that demonstrates how attackers can downgrade HTTPS connections to HTTP to intercept sensitive data. Based on Moxie Marlinspike's original SSLstrip concept, it includes a transparent proxy, ARP spoofing capabilities, and advanced MITM features for authorized security testing.

## Key Features

1. **SSL/TLS Stripping Proxy**:
   - Downgrades HTTPS to HTTP connections
   - Removes HSTS (HTTP Strict Transport Security) headers
   - Strips Secure flag from cookies
   - Replaces HTTPS links with HTTP in responses

2. **Man-in-the-Middle (MITM) Capabilities**:
   - ARP spoofing for network-level interception
   - Transparent proxy operation
   - iptables redirection for Linux systems

3. **Data Capture**:
   - Extracts credentials (passwords, tokens, etc.)
...
4. **Advanced SSL MITM**:
   ...
   - Full SSL interception with proper certificates
...
2. Attacker intercepts and modifies request to http://example.com
3. Attacker connects to real server via HTTPS
4. Server responds with HTTPS content
5. Attacker strips SSL and sends HTTP to victim
6. Victim sees HTTP page (often with mixed content warnings)
7. All subsequent traffic is in cleartext
```
...
```
Original HTTPS Response (with HSTS):
HTTP/1.1 200 OK
...
### Class: `SSLStripProxy`
Main SSL stripping implementation:

- **`start_proxy()`**: Main proxy server loop
- **`handle_http_request()`**: Processes and forwards HTTP requests
- **`strip_ssl()`**: Removes SSL/TLS from responses
- **`extract_sensitive_data()`**: Captures credentials
- **`arp_poisoning_loop()`**: ARP spoofing for MITM
- **`replace_https_links()`**: Modifies HTTPS links to HTTP

### Class: `SSLMITMProxy`
Advanced SSL interception with certificates:

- **`create_ssl_context()`**: Sets up SSL with certificates
- **`handle_https_connection()`**: Manages HTTPS MITM
- **`start_proxy()`**: SSL-aware proxy server
...
### Root Privileges
```bash
# Script requires root for raw sockets and iptables
sudo python3 ssl_strip.py [options]
```
...
### Basic SSL Stripping
```bash
# Simple proxy on port 8080
sudo python3 ssl_strip.py --port 8080

# With verbose output and logging
sudo python3 ssl_strip.py --port 8080 --verbose --log capture.log
```

### Man-in-the-Middle Attack
```bash
# With ARP spoofing (target specific victim)
sudo python3 ssl_strip.py --port 8080 --arp --gateway 192.168.1.1 --target 192.168.1.100

# With iptables redirection (all traffic)
sudo python3 ssl_strip.py --port 8080 --iptables
```

### Advanced Configuration
```bash
# Disable HSTS removal (for testing)
sudo python3 ssl_strip.py --port 8080 --no-hsts

# Keep Secure flag in cookies
sudo python3 ssl_strip.py --port 8080 --no-secure

# Full attack chain with all options
sudo python3 ssl_strip.py --port 8080 --arp --gateway 192.168.1.1 --target 192.168.1.100 \
                         --log sslstrip.log --verbose
```

### SSL MITM with Certificates
```bash
# Generate self-signed certificate
openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes

# Run SSL MITM proxy
sudo python3 ssl_strip.py --mitm --port 8443 --cert cert.pem --key key.pem \
                         --target-host example.com
```

## HSTS and Secure Flags

### HSTS (HTTP Strict Transport Security)
HSTS is a security mechanism that forces browsers to use HTTPS. SSLstrip bypasses it by:
- Removing `Strict-Transport-Security` headers
- Downgrading the first request before HSTS is set
- Using HTTP for subsequent requests

### Secure Cookie Flag
The Secure flag prevents cookies from being sent over HTTP. SSLstrip removes this flag, allowing cookie capture.

## Security Considerations

⚠️ **IMPORTANT LEGAL AND ETHICAL NOTES:**

1. **Authorization Required**: This tool is for authorized penetration testing only. Unauthorized use may violate computer fraud laws and privacy regulations.

2. **HSTS Preloading**: Modern browsers have HSTS preload lists that make SSL stripping ineffective for many sites (google.com, facebook.com, etc.).

3. **Certificate Pinning**: Some applications use certificate pinning, preventing MITM even with valid certificates.

4. **Mixed Content Warnings**: Victims will see mixed content warnings in browsers when HTTPS content is loaded over HTTP.

5. **Responsible Use**: Only test systems you own or have written permission to test.

## Detection and Prevention

### For Users:
1. **Use HTTPS Everywhere browser extension**
2. **Enable HSTS preloading in browsers**
3. **Look for HTTPS in address bar**
4. **Avoid using public Wi-Fi without VPN**
5. **Check for certificate warnings**

### For Network Defenders:
1. **Enable HSTS preloading on your sites**
...
5. **Detect SSL stripping**:
   ```bash
   # Monitor for HTTP/HTTPS anomalies
   tcpdump -i eth0 'tcp port 80 or tcp port 443' -A | grep -i "strict-transport-security"
   ```
...
| ARP spoofing not working | Check gateway IP, ensure on same subnet |
| No traffic captured | Verify proxy settings in victim browser |
| HSTS still present | Site may be in HSTS preload list |
| Certificate warnings | Browser detecting MITM |
| iptables errors | Run on Linux, check permissions |

## Limitations

- **HSTS Preload**: Sites in HSTS preload list are protected
- **Certificate Pinning**: Apps with pinning will fail
- **HTTP/2**: Some implementations behave differently
- **QUIC/HTTP3**: Google's QUIC bypasses the proxy
- **Browser Warnings**: Users may see security warnings

## Educational Use

This tool is excellent for learning about:
- HTTPS/TLS protocol weaknesses
...
To test defenses:
1. Set up test website with HTTPS
2. Enable HSTS with preloading
3. Implement certificate pinning
4. Run SSL stripping attack
5. Observe which protections work
