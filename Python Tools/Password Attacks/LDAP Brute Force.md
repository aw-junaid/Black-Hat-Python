...
## Overview
This is a comprehensive LDAP brute force testing tool designed for security professionals to assess LDAP authentication security. It supports multiple attack methods, user enumeration, and various authentication types.

## Key Features

1. **Multiple Attack Modes**: Standard brute force, password spraying, user enumeration
2. **Authentication Types**: Simple bind and NTLM authentication
3. **Smart Username Formatting**: Automatically tries different username formats (UPN, down-level, DN)
4. **Anonymous Bind Testing**: Checks for misconfigured anonymous access
5. **Server Information Gathering**: Retrieves LDAP server details
6. **User Enumeration**: Extracts user accounts from directory
7. **Rate Limiting**: Configurable delays to avoid account lockouts
8. **Multi-threading**: Concurrent testing for faster results

## Script Structure

### Class: `LDAPBruteForcer`
The main class handling all LDAP operations:

- **`__init__()`**: Initializes connection parameters
- **`test_anonymous_bind()`**: Checks if anonymous bind is allowed
- **`get_server_info()`**: Retrieves server information
- **`enumerate_users()`**: Discovers user accounts
- **`discover_base_dn()`**: Automatically finds base DN
- **`format_username()`**: Formats usernames for different auth types
- **`test_credentials()`**: Tests single credential pair
- **`worker()`**: Thread worker for concurrent testing
- **`brute_force()`**: Main brute force attack function
- **`password_spray()`**: Password spraying attack
- **`save_results()`**: Exports found credentials
...
# For NTLM authentication support (included in ldap3)
```

### Setup
1. Save the script as `ldap_bruteforce.py`
2. Make it executable: `chmod +x ldap_bruteforce.py`
3. Install dependency: `pip install ldap3`
...
python ldap_bruteforce.py ldap.example.com --info -v
...
python ldap_bruteforce.py ldap.example.com -U users.txt -P passwords.txt -t 10

# Brute force with specific credentials
python ldap_bruteforce.py ldap.example.com -U "admin,user1,user2" -P "password,123456,admin" -t 5

# NTLM authentication
python ldap_bruteforce.py ldap.example.com -U users.txt -P passwords.txt --domain example.com --ntlm

# With rate limiting to avoid lockouts
python ldap_bruteforce.py ldap.example.com -U users.txt -P passwords.txt --delay 2
```

### Password Spraying
```bash
# Single password against many users
python ldap_bruteforce.py ldap.example.com -U users.txt --spray "Spring2024!"

# With custom port and SSL
python ldap_bruteforce.py ldap.example.com -p 636 -s -U users.txt --spray "Welcome1"
```

### Advanced Usage
```bash
# Full attack chain
python ldap_bruteforce.py ldap.example.com \
  --info \
  --anonymous \
  --enumerate -o discovered_users.txt \
  -U discovered_users.txt \
...
⚠️ **IMPORTANT LEGACY AND ETHICAL NOTES:**
- **This tool should only be used on systems you own or have explicit permission to test**
- Unauthorized access attempts may be illegal in your jurisdiction
- Password spraying can trigger account lockouts - use carefully
- Always follow responsible disclosure practices
- Document all testing activities with proper authorization

## Features Explained

### Username Formatting
The tool automatically tries multiple username formats:
- `username` - Simple username
- `username@domain.com` - User Principal Name (UPN)
- `DOMAIN\username` - Down-level logon name
- `cn=username,dc=domain,dc=com` - Distinguished Name (DN)

### Attack Types

1. **Standard Brute Force**: Tests all username/password combinations
2. **Password Spraying**: Tests one password against many users (lower detection risk)
3. **User Enumeration**: Discovers valid usernames before attacking

### Anti-Detection Features
- Configurable delay between attempts
- Single-threaded option for password spraying
- Random jitter in timing (optional)
- Multiple authentication format attempts

## Performance Optimization

- **Threads**: Adjust based on server capacity (5-20 typical)
- **Delay**: 1-2 seconds for password spray, 0 for internal testing
- **Wordlist size**: Start with small, targeted lists
...
1. **"ldap3 module not found"**: Install with `pip install ldap3`
2. **Connection refused**: Check port and firewall settings
3. **Invalid credentials format**: Try different username patterns
4. **SSL errors**: Use `-p 389` without SSL or verify certificates
5. **LDAP injection**: Tool uses proper escaping (not vulnerable)

## Limitations

- No Kerberos authentication support
- Basic rate limiting (no exponential backoff)
- No distributed attack capability
- Single domain only (no forest trust traversal)
