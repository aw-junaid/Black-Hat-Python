# Python SMB Brute Force Tool with Impacket
...
class SMBBruteForcer:
    ...
    # Impacket-specific SMB connection parameters 
    ...
    def test_credentials_impacket(self, username: str, password: str, 
        ...
        try:
            ...
            # Create SMB connection with specified dialect 
    ...
    def password_spray(self, usernames: List[str], password: str,
        ...
        Uses fewer threads and longer delays to avoid lockouts 
...
Examples:
  ...
  python smb_bruteforce.py 192.168.1.100 -U users.txt -P passwords.txt --domain EXAMPLE --dc
    ...
    if not IMPACKET_AVAILABLE:
        ...
        print("    On Kali: sudo apt install impacket-scripts ")
        sys.exit(1)
...
## Overview
This is a comprehensive SMB brute force testing tool built with the Impacket library . It provides enterprise-grade SMB authentication testing with support for multiple SMB dialects, domain authentication, Kerberos, and intelligent attack patterns designed for authorized security assessments.

## Key Features

1. **Multiple SMB Dialects**: Supports SMB1, SMB2, and SMB3 protocols 
2. **Authentication Methods**: NTLM and Kerberos authentication 
3. **Domain Integration**: Full Active Directory domain support
4. **Smart Attack Modes**:
...
5. **Advanced Capabilities**:
   - Share enumeration after successful login
...
### Class: `SMBBruteForcer`
The main class handling all SMB operations with Impacket:

- **`__init__()`**: Initializes connection parameters and validates target
- **`get_smb_info()`**: Retrieves SMB server information (OS, domain, signing) 
- **`test_credentials_impacket()`**: Tests credentials using Impacket's SMBConnection
- **`check_null_session()`**: Tests for anonymous/null session access
- **`enumerate_shares()`**: Lists available shares with valid credentials
- **`worker()`**: Thread worker for concurrent testing
- **`brute_force()`**: Main attack orchestrator
- **`password_spray()`**: Low-and-slow password spraying 
- **`domain_brute_force()`**: Specialized domain authentication attacks
- **`save_results()`**: Exports findings with share information
...
# For Kerberos support (usually included with impacket)
pip install pyasn1 pycryptodomex
```
...
```bash
# Standard attack with wordlists
python smb_bruteforce.py 192.168.1.100 -U users.txt -P passwords.txt

# Single username
python smb_bruteforce.py 192.168.1.100 -u administrator -P passwords.txt
```
...
python smb_bruteforce.py 192.168.1.100 -U users.txt --spray "Summer2024!" --delay 3 -t 3

# Domain password spray
python smb_bruteforce.py 192.168.1.100 -U users.txt --spray "Welcome1" --domain EXAMPLE
```
...
```bash
# Complete assessment
python smb_bruteforce.py 192.168.1.100 \
  --null-session \
  --common \
  -U users.txt \
  --spray "Autumn2024!" \
...
## Impacket Integration Details

### SMBConnection Usage
The tool leverages Impacket's `SMBConnection` class for robust protocol handling :

```python
# Key Impacket features used 
...
# Kerberos authentication
--kerberos flag + domain parameter
```
...
1. **Authorization Required**: This tool is for authorized penetration testing only . Unauthorized access attempts violate computer fraud laws.

2. **Account Lockout Risk**: SMB brute force can trigger account lockouts. Password spraying with delays is recommended .

3. **Detection**: SMB attacks generate Windows Event ID 4625 (failed logons) and may trigger security alerts.

4. **Responsible Use**: Only test systems you own or have written permission to test.

## How Impacket Powers the Tool

### SMB Protocol Implementation
Impacket provides low-level access to SMB packets and protocol implementations :

1. **Connection Establishment**: Negotiates SMB dialect
2. **Authentication**: Handles NTLM challenge/response
3. **Session Management**: Maintains authenticated sessions
4. **Share Access**: Enumerates and accesses shares

### Error Handling
Impacket provides detailed NT status codes:
...
- **Thread Count**: 3-5 for password spray, 5-10 for brute force
- **SMB Version**: SMB2/SMB3 for modern systems, SMB1 for legacy
- **Delay Settings**: 1-3 seconds for password spray
- **Domain Targeting**: Use domain parameter for faster auth

## SMB Dialect Comparison

| Dialect | Features | Use Case |
|---------|----------|----------|
| SMB1 | Legacy, vulnerable | Old systems (pre-2007) |
| SMB2 | Faster, more secure | Windows 7/Server 2008+ |
| SMB3 | Encryption, multichannel | Modern Windows  |

## Mitigation Recommendations

For system administrators :

1. **Disable SMB1**: Legacy protocol is vulnerable
2. **Require SMB Signing**: Prevents relay attacks
3. **Implement Account Lockout**: After 5-10 failed attempts
4. **Monitor Event Logs**: Watch for 4625 (failed logon) events
5. **Use Strong Passwords**: Enforce complexity
6. **Disable Null Sessions**: Prevent anonymous access
7. **Network Segmentation**: Limit SMB traffic to trusted networks

## Troubleshooting

| Issue | Solution |
|-------|----------|
| "impacket not found" | `pip install impacket` |
| Connection refused | Check port (445 or 139) |
| SMB signing required | Use valid domain credentials  |
| Account lockouts | Reduce threads, add delay |
| Kerberos errors | Ensure DNS and time sync |
| SMB3 failures | Fallback to SMB2 |

## Limitations

- No SMB over QUIC support
- Limited to password-based authentication
- No pass-the-hash in this version
- Kerberos requires proper DNS configuration 
