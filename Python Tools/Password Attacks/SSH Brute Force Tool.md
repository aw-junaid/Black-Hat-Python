# Python SSH Brute Force Tool with Paramiko
...
## Overview
This is a comprehensive SSH brute force testing tool built with the Paramiko library. It provides multiple attack modes, intelligent threading, and extensive logging capabilities for authorized security testing .

## Key Features

1. **Multiple Attack Modes**:
   - Standard brute force (multiple usernames + multiple passwords)
   - Dictionary attack (single username + password list)
   - Username brute force (multiple usernames + single password)
   - Password variation generation 

2. **Advanced Capabilities**:
   - Multi-threading support for faster attacks 
   - Custom SSH port configuration 
   - Connection timeout handling
   - SSH banner grabbing
   - Color-coded output with colorama support

3. **Intelligent Features**:
   - Automatic password variation generation
   - Progress tracking with real-time statistics
   - Configurable stop-on-success behavior
...
### Class: `SSHBruteForcer`
The main class handling all SSH operations:
...
- **`username_brute()`**: Username brute force with single password
- **`generate_common_passwords()`**: Creates password variations from base words 
...
- **Linux/Mac**: Standard pip installation works
- **Windows**: May need additional dependencies for Paramiko (typically installed automatically)
...
# With custom port
python ssh_bruteforce.py 192.168.1.100 -p 2222 -u admin -P passwords.txt
```
...
python ssh_bruteforce.py 192.168.1.100 -U "root,admin,user" -P "password,123456,admin"
```

### Advanced Attacks
```bash
# Username brute force (find valid usernames)
python ssh_bruteforce.py 192.168.1.100 --brute-users users.txt -p "Password123"

# Generate password variations
python ssh_bruteforce.py 192.168.1.100 -u admin --generate -t 8

# Full attack with result logging
python ssh_bruteforce.py 192.168.1.100 -U users.txt -P passwords.txt -t 20 -v -o results.txt
```

### Banner Grabbing
```bash
# Get SSH server information
python ssh_bruteforce.py 192.168.1.100 -u root -P passwords.txt --banner
```
...
2. **Rate Limiting**: The tool includes configurable delays to avoid overwhelming target servers and triggering DoS protections .

3. **Logging**: All attempts are logged for audit purposes - maintain these logs for authorization verification.
...
The tool uses Paramiko's SSHClient to establish connections :
1. Creates SSH client with AutoAddPolicy for host key handling
2. Attempts connection with provided credentials
3. Validates successful authentication by testing command execution
4. Properly closes connections to avoid resource leaks

### Threading Model
- Uses producer-consumer pattern with Queue for thread safety 
- Each worker thread processes credentials independently
...
- Resource cleanup on interruptions

## Performance Optimization

- **Thread Count**: Start with 5-10 threads, adjust based on server response
- **Timeout Settings**: Default 5 seconds, increase for slow connections
- **Wordlist Size**: Begin with targeted lists before expanding
- **Stop on Success**: Enable to save time once credentials are found

## Mitigation Recommendations

For system administrators, here are key protections against SSH brute force attacks :

1. **Disable Root Login**: Set `PermitRootLogin no` in `/etc/ssh/sshd_config`
2. **Use Key-Based Authentication**: Prefer SSH keys over passwords
3. **Implement Rate Limiting**: Use tools like fail2ban
4. **Configure MaxStartups**: Set `MaxStartups 10:30:60` in sshd_config
5. **Use Non-Standard Ports**: Change from default port 22
6. **Strong Password Policies**: Enforce complex passwords
...
| False negatives | Verify target is running SSH service |

## Limitations

- No support for keyboard-interactive authentication 
- Cannot handle multi-factor authentication
- Limited to password-based authentication
- No distributed attack capability
- Basic key-based authentication only
