# Python WHOIS Lookup Automation Tool
...
## Overview
This is a comprehensive WHOIS lookup automation tool that provides detailed information about domains, IP addresses, and AS numbers. It supports bulk lookups, multiple export formats, caching, and monitoring capabilities for security research, OSINT gathering, and network intelligence.

## Key Features

1. **Multi-Type WHOIS Lookups**:
   - Domain name WHOIS
   - IP address WHOIS
   - AS number WHOIS
...
### Class: `WHOISParser`
Parses raw WHOIS text into structured data:

- **`parse_domain_whois()`**: Extracts domain registration details
- **`parse_ip_whois()`**: Extracts IP allocation information
- **`parse_as_whois()`**: Extracts AS number details
- **`FIELD_PATTERNS`**: Regex patterns for common WHOIS fields

### Class: `WHOISLookup`
Main WHOIS lookup functionality:

- **`lookup_domain()`**: Domain WHOIS with library/socket fallback
- **`lookup_ip()`**: IP WHOIS with RIR referral following
- **`lookup_asn()`**: AS number WHOIS
- **`bulk_lookup()`**: Multi-threaded batch processing
- **`socket_whois()`**: Raw TCP socket WHOIS client
- **`export_results()`**: Multiple format exporters

### Class: `WHOISMonitor`
Domain monitoring for changes:

- **`monitor_domain()`**: Periodic WHOIS checks
- **Change detection**: Compares key fields over time
...
pip install colorama tqdm pandas requests asyncwhois

# On Debian/Ubuntu
sudo apt install python3-whois python3-colorama python3-tqdm python3-pandas
```
...
### Protocol Basics
- **Port**: 43 (TCP)
- **Query**: Send domain + CRLF
- **Response**: Text data, ends with empty line
- **Referrals**: Some servers redirect to authoritative WHOIS

### Query Types
```
Domain: example.com
IP: 8.8.8.8 or 192.168.0.0/16
ASN: AS15169 or 15169
```

### Server Hierarchy
```
IANA (whois.iana.org) → TLD WHOIS (whois.verisign-grs.com) → Registrar WHOIS
                          ↓
                    RIR WHOIS (arin.net, ripe.net, etc.)
```
...
```bash
# Bulk lookup from file
python whois_lookup.py --file domains.txt --output results.json

# Export to CSV
python whois_lookup.py --file domains.txt --output results.csv --format csv

# Multi-threaded with delay
python whois_lookup.py --file domains.txt --threads 10 --delay 0.2
```
...
# Disable cache
python whois_lookup.py --domain example.com --no-cache

# Verbose with raw output
python whois_lookup.py --domain example.com --verbose
```

### Monitoring
```bash
# Monitor domain for changes (check every 24 hours)
python whois_lookup.py --monitor example.com

# Check every hour for one day
python whois_lookup.py --monitor example.com --interval 3600 --duration 86400
```
...
- **Threading**: Parallel lookups for bulk operations
- **Caching**: Avoid repeated queries
- **Delays**: Prevent rate limiting
- **Timeouts**: Handle unresponsive servers

## Troubleshooting

| Issue | Solution |
|-------|----------|
| Connection refused | Check WHOIS server, port 43 |
| Timeout | Increase timeout value |
| No data returned | Try different WHOIS server |
| Parsing errors | Check raw output format |
| Rate limited | Add delays between queries |
| Cache not working | Check file permissions |

## Limitations

- WHOIS rate limits from servers
- Inconsistent data formats across registrars
- Some domains have privacy protection
- Not all TLDs support WHOIS
- IPv6 WHOIS may be limited

## Best Practices

1. **Respect Rate Limits**: Add delays between queries
2. **Use Caching**: Reduce server load
3. **Handle Errors**: Graceful failure handling
4. **Validate Input**: Ensure proper query format
5. **Follow Terms**: Comply with WHOIS server terms

## Educational Use

This tool is excellent for learning about:
- WHOIS protocol and infrastructure
